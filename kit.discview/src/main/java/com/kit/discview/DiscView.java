...
import com.kit.discview.utils.DisplayUtil;
...
public class DiscView extends RelativeLayout {
    ...
    public void setMusicRes(int musicRes){
        ...
        ImageView disc = (ImageView) discLayout.findViewById(R.id.ivDisc);
        disc.setImageDrawable(getDiscDrawable(musicRes));

        mDiscAnimators.add(getDiscObjectAnimator(disc, 0));
        mDiscLayouts.add(discLayout);
        mViewPagerAdapter.notifyDataSetChanged();
        if (mIPlayInfo != null) {
            mIPlayInfo.onMusicPicChanged(musicRes);
        }
    }
