...
public class StatusBarUtils {
    ...
    /**
     * 是否是最外层布局DrawerLayout的侧滑菜单
     * @param drawerLayout 是否是最外层布局DrawerLayout
    ...
    private int getStatusBarHeight(Activity activity) {
        int result = 0;
        //获取状态栏高度的id
        int resourcesId = activity.getResources().getIdentifier("status_bar_height", "dimen", "android");
        if (resourcesId > 0){
            result = activity.getResources().getDimensionPixelSize(resourcesId);
        }
        return result;
    }
